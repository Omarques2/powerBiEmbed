...
describe("PowerBiService", () => {
  ...
  it("includes identities when opts are provided", async () => {
    mockedAxios.get.mockImplementation(async (url: string) => {
      if (url.includes("/reports/")) {
        return { data: { embedUrl: "https://embed", datasetId: "dataset-1" } } as any;
      }
      if (url.includes("/datasets/")) {
        return { data: { isEffectiveIdentityRequired: true, isEffectiveIdentityRolesRequired: true } } as any;
      }
    ...
    mockedAxios.post.mockImplementation(async (url: string, body: any) => {
      ...
      if (url.endsWith("/GenerateToken")) {
        ...
        return { data: { token: "embed-token", expiration: "2099-01-01T00:00:00Z" } } as any;
      }
      throw new Error("unexpected url");
    });
  ...
  it("omits identities when opts are not provided", async () => {
    mockedAxios.get.mockImplementation(async (url: string) => {
      if (url.includes("/reports/")) {
        return { data: { embedUrl: "https://embed", datasetId: "dataset-2" } } as any;
      }
      if (url.includes("/datasets/")) {
        return { data: { isEffectiveIdentityRequired: false, isEffectiveIdentityRolesRequired: false } } as any;
      }
      throw new Error("unexpected url");
    });
