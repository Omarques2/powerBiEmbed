...
export class AdminRlsController {
  ...
  deleteTarget(@Req() req: AuthedRequest, @Param("targetId", ParseUUIDPipe) targetId: string) {
    const actorSub = req.user?.sub ? String(req.user.sub) : null;
  ...
  @Delete("rules/:ruleId")
  deleteRule(@Req() req: AuthedRequest, @Param("ruleId", ParseUUIDPipe) ruleId: string) {
    const actorSub = req.user?.sub ? String(req.user.sub) : null;
    return this.svc.deleteRule(ruleId, actorSub);
  }

  @Post("datasets/:datasetId/refresh")
  refreshDataset(@Req() req: AuthedRequest, @Param("datasetId", ParseUUIDPipe) datasetId: string) {
    const actorSub = req.user?.sub ? String(req.user.sub) : null;
