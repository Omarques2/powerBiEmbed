...
import { INestApplication, ForbiddenException, UnauthorizedException, ValidationPipe } from "@nestjs/common";
...
describe("AuthZ guard coverage", () => {
  ...
  it("returns 403 when PlatformAdminGuard blocks admin endpoints", async () => {
    const app = await createApp({
      ...
      platformAdminGuard: {
        canActivate: () => {
          throw new ForbiddenException({ code: "NOT_PLATFORM_ADMIN", message: "Not platform admin" });
        },
      },
    });
  ...
  it("returns 401 when AuthGuard blocks admin RLS endpoints", async () => {
    ...
    const res = await request(app.getHttpServer()).get(`/admin/rls/datasets/${datasetId}/targets`);
    expect(res.status).toBe(401);
    expect(res.body.error.code).toBe("UNAUTHORIZED");
    expect(res.body.correlationId).toBeTruthy();
